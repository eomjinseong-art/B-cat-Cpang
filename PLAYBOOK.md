...
싱크와 홈 목록은 최대 400개까지 담는다. 중복·이미지 없음 행은 그 안에서 빠질 수 있다. 숨숨마을은 18번이 30번과 같은 상품이라 `sheet-flags.json`의 `duplicates: [18]`로 남아 있다.
...
- 홈 목록은 한 페이지 50개, 하단 번호 페이지네이션. 순서는 시트 `id` 기준이며 페이지를 넘겨도 다시 섞지 않는다.
